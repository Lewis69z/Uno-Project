...
* 2- Power cards: like Red skip, blue reverse ( Color + skip, blue, reverse ( Color = color , type = (skip, reverse,plustwo) NUM = -1)
* 3- wild cards: Colorchange or Plusfour ( color = "wild" , type = Plusfour, Colorchange , NUM = -1)
* ******************************************************************************************************************************************************
* ---------------------Difference between normal - hard - insane-------------------------
* well, Noraml : It just play like a little bit random ( Priority : Normal cards > wild cards), just finds the first card could be played and don't think what gonna happen next
...
#define MAX 1000	//Stack array size
int start = 0;	//Used @ void game
...
// Computer or Player ? normal - hard - insane
void Decide()
{
	cout << " Note : " << endl << "     - 'Three' options are avilable for computer : NORMAL, HARD and INSANE mode!\n     -  Please type small letters only!\n" << endl;
...
repeat:
	...
	for (int i = 0; i < 20; i++)
	{
		if (isalpha(choose[i]) && isalpha(choose[strlen(choose) - i - 1]))
		{
			...
			{
				cout << " \nPlease enter a correct answer ( computer - secondplayer )" << endl << " Your Answer : ";
				goto repeat;
			}
		}
		else
	...
	{
		cout << "\n Choose the Difficulty level (normal - hard - insane)\n Your Answer : ";
	repeat2:
		...
		for (int i = 0; i < 20; i++)
		{
			if (isalpha(choose[i]) && isalpha(choose[strlen(choose) - i - 1]))
			{
				if (strcmp(difficulty, "normal") == 0 || strcmp(difficulty, "hard") == 0 || strcmp(difficulty, "insane") == 0)
				{
					break;
				}
				else
				{
					cout << " \nPlease enter a correct answer ( normal - hard - insane )" << endl << " Your Answer : ";
					goto repeat2;
				}
			}
			else
				...
				if (call.cardss[pcr].Color == gcc)
				{
					if (call.cardss[pcr].NUM == -1)
					{
						if (call.cardss[pcr].Color == "red")
						{
							...
							else if (call.cardss[pcr].type == "plustwo")
							{
								if (gt >= 1 || yt >= 1 || bt >= 1)
								{
									if (gt >= 1)
...
int main()
{
	...
	cout << "				Mariam Taher Abdel-Aziz			Section: 4		BN: 9	\n";
	cout << "				Mariam Saeed Hamdy		        Section: 4		BN: 8	\n\n";
	...
	cout << "							Start date: 4/14/2022 -------End Date: 4/22/2022";
	//Initializing..
	call.InitializeH();
